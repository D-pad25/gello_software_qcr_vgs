...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    # xArm
    # "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT9HDFUF-if00-port0": DynamixelRobotConfig(
    #     joint_ids=(1, 2, 3, 4, 5, 6),
    #     joint_offsets=(
    #         6*np.pi/2, #was 2 before 
    #         3*np.pi/2, 
    #         1*np.pi/2, 
    #         3*np.pi/2, 
    #         3*np.pi/2, 
    ...
    #     gripper_config=(7, 275, 233),
    # ),

    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT9HDFUF-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6),
        joint_offsets=(
            0*np.pi/2, #was 2 before 
            3*np.pi/2, 
            1*np.pi/2, 
            3*np.pi/2, 
            3*np.pi/2, 
...
# [0*np.pi/2, 3*np.pi/2, 2*np.pi/2, 3*np.pi/2, 3*np.pi/2, 1*np.pi/2 ]
