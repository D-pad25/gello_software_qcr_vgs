...
class SensorProcessor:
    ...
    def _extract_sensor_data(self, data):
        ...
        try:
            group1_raw = [int(value, 16) for value in data["2"]["data"].split(",")]
            group2_raw = [int(value, 16) for value in data["1"]["data"].split(",")]
    ...
    def start_websocket(self):
        ...
        try:
            ...
            mp.Thread(target=ws.run_forever, daemon=True).start()
        except Exception as e:
            print(f"Failed to start WebSocket: {e}")


    def start(self):
        """Start the WebSocket and optionally the live plot."""
